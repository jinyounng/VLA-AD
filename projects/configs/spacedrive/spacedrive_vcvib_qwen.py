...
############### Vision-Conditioned VIB config ###############
vib_beta = 1e-3              # KL weight — primary tuning knob
vib_z_dim = None            # None → same as llm_hidden_dim (no dim reduction)
vib_prior_hidden_dim = None  # None → vision_dim // 2 (PriorNet hidden dim)

# save paths
base_path = 'workspace/spacedrive_vcvib_qwen/'
work_dir = base_path
...
model = dict(
    ...
    vib_beta=vib_beta,
    vib_z_dim=vib_z_dim,
    vib_prior_hidden_dim=vib_prior_hidden_dim,
)
