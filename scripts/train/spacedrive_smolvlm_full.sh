#!/usr/bin/env bash
# SpaceDrive + SmolVLM full fine-tuning launcher.
...
# Full FT defaults (override via env if needed)
LR="${LR:-5e-5}"   # keep independent from Qwen baseline
BATCH_SIZE="${BATCH_SIZE:-4}"
ACCUM_ITERS="${ACCUM_ITERS:-4}"
NUM_EPOCHS="${NUM_EPOCHS:-6}"
WORK_DIR="${WORK_DIR:-workspace/spacedrive_smolvlm_full_plus}"
...
LOG_INTERVAL="${LOG_INTERVAL:-1}"  # default: 1 epoch
