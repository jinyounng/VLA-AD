#!/usr/bin/env bash
# Distributed planning inference with runtime-only feature+PE ego order fix.
# Original model/source files are not modified.
...
python3 -m torch.distributed.launch \
  ...
  --nproc_per_node="${GPUS}" \
  --master_port="${PORT}" \
  "${SCRIPT_DIR}/test_fix_ego_feature_pe_order.py" \
  "${CONFIG}" \
  "${CHECKPOINT}" \
  --seed "${SEED}" \
  --launcher pytorch \
