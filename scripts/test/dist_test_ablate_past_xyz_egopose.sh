#!/usr/bin/env bash
# Distributed VLM planning inference with past xyz/PE + full past ego-pose ablated.
...
echo "ABLATION=past_xyz_egopose_zero"
...
python3 -m torch.distributed.launch \
  ...
  --nproc_per_node="${GPUS}" \
  --master_port="${PORT}" \
  "${SCRIPT_DIR}/test_ablate_past_xyz_egopose.py" \
  "${CONFIG}" \
  "${CHECKPOINT}" \
  --seed "${SEED}" \
  --launcher pytorch \
