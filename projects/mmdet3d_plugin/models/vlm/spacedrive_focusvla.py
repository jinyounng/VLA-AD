...
from ..vlm_utils.misc import load_model as load_baseline_model, locations
from ..vlm_utils.custom_qwen_focusvla import CustomQwen2_5_VLFocusVLAForConditionalGeneration
...
def load_focusvla_model(
    base_model,
    tokenizer_path,
    use_lora,
    frozen,
    llm_lora_rank=16,
    focus_vla_enabled=False,
    focus_vla_top_k=64,
    focus_vla_query_mode="mean",
    focus_vla_fusion_mode="gated_residual",
    focus_vla_alpha=1.0,
):
    if not ('Qwen2.5' in base_model):
        return load_baseline_model(base_model, tokenizer_path, use_lora, frozen, llm_lora_rank=llm_lora_rank)
    ...
    model = CustomQwen2_5_VLFocusVLAForConditionalGeneration.from_pretrained(
        base_model,
    ...
    model.configure_focus_vla(
        enabled=focus_vla_enabled,
        top_k=focus_vla_top_k,
        query_mode=focus_vla_query_mode,
        fusion_mode=focus_vla_fusion_mode,
        alpha=focus_vla_alpha,
    )
    ...
    if use_lora:
        ...
        for name, param in model.named_parameters():
            if 'coords' in name or 'focused_visual_context' in name:
...
class SpaceDriveFocusVLA(MVXTwoStageDetector):
    """SpaceDrive FocusVLA experimental variant."""
    def __init__(self,        
        ...
        super(SpaceDriveFocusVLA, self).__init__(train_cfg, test_cfg,)
        ...
        self.focus_vla_enabled = focus_vla_enabled
        self.focus_vla_top_k = focus_vla_top_k
        self.focus_vla_query_mode = focus_vla_query_mode
        self.focus_vla_fusion_mode = focus_vla_fusion_mode
        self.focus_vla_alpha = focus_vla_alpha
        ...
        if lm_head is not None:
            self.lm_head = load_focusvla_model(
                lm_head,
                tokenizer,
                use_lora,
                frozen,
                llm_lora_rank=llm_lora_rank,
                focus_vla_enabled=focus_vla_enabled,
                focus_vla_top_k=focus_vla_top_k,
                focus_vla_query_mode=focus_vla_query_mode,
                focus_vla_fusion_mode=focus_vla_fusion_mode,
                focus_vla_alpha=focus_vla_alpha,
            )
    ...
    def forward_train_vlm(self,
        ...
        if self.with_lm_head:
            ...
            focus_vla_stats = getattr(lm_loss, 'focus_vla_stats', None)
            if focus_vla_stats is not None:
                losses.update(
                    focus_vla_topk_score_mean=focus_vla_stats['topk_score_mean'],
                    focus_vla_attn_entropy=focus_vla_stats['attn_entropy'],
                    focus_vla_context_norm=focus_vla_stats['context_norm'],
                    focus_vla_q_action_norm=focus_vla_stats['q_action_norm'],
                    focus_vla_gate_mean=focus_vla_stats['gate_mean'],
                )
            if self.io_3d_pos: 
