...
from ...datasets.utils.constants import (
    IGNORE_INDEX, IMAGE_TOKEN_INDEX, VISION_START_TOKEN_INDEX, VISION_END_TOKEN_INDEX,
    POS_INDICATOR_TOKEN_INDEX, POS_EMBEDDING_TOKEN_INDEX, POS_EMBEDDING_TOKEN, POS_INDICATOR_TOKEN,
    SMOLVLM_IMAGE_TOKEN_INDEX, SMOLVLM_POS_INDICATOR_TOKEN_INDEX, SMOLVLM_POS_EMBEDDING_TOKEN_INDEX,
)
...
class SpaceDriveSmolVLM(MVXTwoStageDetector):
    """SpaceDrive."""
    def __init__(self,        
        ...
        super(SpaceDriveSmolVLM, self).__init__(train_cfg, test_cfg,)
        ...
        if 'llava' in processor:
            ...
            self.lm_type = 'smolvlm'

        if self.lm_type == 'smolvlm':
            self._image_token_index = SMOLVLM_IMAGE_TOKEN_INDEX
            self._pos_indicator_token_index = SMOLVLM_POS_INDICATOR_TOKEN_INDEX
            self._pos_embedding_token_index = SMOLVLM_POS_EMBEDDING_TOKEN_INDEX
        else:
            self._image_token_index = IMAGE_TOKEN_INDEX
            self._pos_indicator_token_index = POS_INDICATOR_TOKEN_INDEX
            self._pos_embedding_token_index = POS_EMBEDDING_TOKEN_INDEX
        ...
        self._lm_model_for_pe = None
        if lm_head is not None:
            self.lm_head = load_model(lm_head, tokenizer, use_lora, frozen, llm_lora_rank=llm_lora_rank )
            # Resolve underlying wrapped modules across PEFT/non-PEFT paths.
            base_model = getattr(self.lm_head, "base_model", None)
            candidates = [
                self.lm_head,
                base_model,
                getattr(self.lm_head, "model", None),
                getattr(base_model, "model", None) if base_model is not None else None,
            ]
            # Add one extra nesting level for wrappers like base_model.model.model
            candidates.extend([getattr(c, "model", None) if c is not None else None for c in candidates])
            candidates = [c for c in candidates if c is not None]

            if self.tokenizer is not None:
                for c in candidates:
                    if hasattr(c, "tokenizer"):
                        c.tokenizer = self.tokenizer
                        break

            # Owner of PE attributes (position encoder, pc range, ...)
            self._lm_model_for_pe = next((c for c in candidates if hasattr(c, "position_encoder")), None)
            # Any module exposing .config can drive hidden-size resolution.
            config_holder = next((c for c in candidates if hasattr(c, "config")), None)
            if 'llava' in lm_head:
                self.llm_hidden_dim = 4096
            elif 'Qwen' in lm_head and config_holder is not None:
                if hasattr(config_holder.config, "hidden_size"):
                    self.llm_hidden_dim = config_holder.config.hidden_size
                elif hasattr(config_holder.config, "text_config") and hasattr(config_holder.config.text_config, "hidden_size"):
                    self.llm_hidden_dim = config_holder.config.text_config.hidden_size
            elif 'SmolVLM' in lm_head or 'Idefics' in lm_head or 'smolvlm' in lm_head:
                if config_holder is not None and hasattr(config_holder.config, "text_config"):
                    self.llm_hidden_dim = config_holder.config.text_config.hidden_size
                elif config_holder is not None and hasattr(config_holder.config, "hidden_size"):
                    self.llm_hidden_dim = config_holder.config.hidden_size
                else:
                    print('Warning: failed to resolve SmolVLM hidden size from config, using default 4096.')
            else:
                self.llm_hidden_dim = 4096
        ...
        if self.io_3d_pos:
            ...
            if lm_head is not None:
                if self._lm_model_for_pe is not None:
                    self._lm_model_for_pe.position_encoder = self.position_encoder
                    self._lm_model_for_pe.pc_range = self.pc_range
                    self._lm_model_for_pe.voxel_size = self.voxel_size
                    self._lm_model_for_pe.pos_emb_grid = self.pos_emb_grid
    ...
    def prepare_location(self, image_grid_thw, pixel_values):
        ...
        if self.lm_type == 'smolvlm':
            grid_h = int(image_grid_thw[0][0][1])
            grid_w = int(image_grid_thw[0][0][2])
            dummy = torch.zeros(bs * n, 1, grid_h, grid_w, device=pixel_values.device)
            location = locations(dummy, self.stride, pad_h, pad_w)[None].repeat(bs * n, 1, 1, 1)
        else:
            x = pixel_values.reshape(bs, n, image_grid_thw[0][0][1], image_grid_thw[0][0][2], -1)
            x = x.permute(0, 1, 4, 2, 3).flatten(0, 1)
    ...
    def forward_train_vlm(self,
                          ...
                          **data):
        if self.lm_type == 'smolvlm':
            # Dataloader may collate processor outputs as (B, 1, N, C, H, W).
            ...
            # In sequence mode, pixel_values can be flattened as (B*T, N, C, H, W).
            # Align to current-frame batch using input_ids batch size.
            if (
                pixel_values is not None
                and pixel_values.dim() == 5
                and input_ids is not None
                and pixel_values.shape[0] != input_ids.shape[0]
                and pixel_values.shape[0] % input_ids.shape[0] == 0
            ):
                bsz = input_ids.shape[0]
                tlen = pixel_values.shape[0] // bsz
                pixel_values = pixel_values.reshape(bsz, tlen, *pixel_values.shape[1:])[:, -1]
            if image_grid_thw is not None and torch.is_tensor(image_grid_thw):
                if image_grid_thw.dim() == 4 and image_grid_thw.shape[1] == 1:
                    image_grid_thw = image_grid_thw.squeeze(1)
                if image_grid_thw.dim() == 2:
                    image_grid_thw = image_grid_thw.unsqueeze(0)
                if (
                    image_grid_thw.dim() == 3
                    and input_ids is not None
                    and image_grid_thw.shape[0] != input_ids.shape[0]
                    and image_grid_thw.shape[0] % input_ids.shape[0] == 0
                ):
                    bsz = input_ids.shape[0]
                    tlen = image_grid_thw.shape[0] // bsz
                    image_grid_thw = image_grid_thw.reshape(bsz, tlen, *image_grid_thw.shape[1:])[:, -1]
        ...
        if self.ego_status is not None:
            ...
            if 'feature' in self.ego_status:
                ...
                if input_ids is not None:
                    if (input_ids[0] == VISION_END_TOKEN_INDEX).sum() == 0:
                        last_vision_end_token = (input_ids[0] == self._image_token_index).nonzero().max()
                    else:
                        last_vision_end_token = (input_ids[0] == VISION_END_TOKEN_INDEX).nonzero().max()

                    if self.lm_type in ('llava', 'smolvlm'):
                        insert_input_ids = torch.tensor([self._image_token_index], device=input_ids.device).unsqueeze(0).repeat(B, 1)
                        insert_labels = torch.tensor([IGNORE_INDEX], device=vlm_labels.device).unsqueeze(0).repeat(B, 1)
                        insert_attn_mask = torch.tensor([True], device=vlm_attn_mask.device, dtype=torch.bool).unsqueeze(0).repeat(B, 1)
                    else:
                        ...
                        insert_attn_mask = torch.tensor([True, True, True], device=vlm_attn_mask.device, dtype=torch.bool).unsqueeze(0).repeat(B, 1)
            ...
            if 'PE' in self.ego_status:
                ...
                if input_ids is not None:
                    ...
                    insert_input_ids = torch.tensor([self._pos_indicator_token_index, self._image_token_index]*len_past_pos, device=input_ids.device).unsqueeze(0).repeat(B, 1)
                    insert_labels = torch.tensor([IGNORE_INDEX]*len_past_pos *2, device = vlm_labels.device).unsqueeze(0).repeat(B, 1)
                    insert_attn_mask = torch.ones((1, len_past_pos * 2), device=vlm_attn_mask.device, dtype=torch.bool).repeat(B, 1)
        ...
        if self.ego_status is not None:
            ...
            if 'feature' in self.ego_status:
                ...
                if input_ids is not None:
                    ...
                    if self.lm_type in ('llava', 'smolvlm'):
                        insert_input_ids = torch.tensor([self._image_token_index], device=input_ids.device).unsqueeze(0)
                        insert_attn_mask = torch.tensor([True], device=input_ids.device, dtype=torch.bool).unsqueeze(0)
                    else:
                        ...
                        insert_attn_mask = torch.tensor([True, True, True], device=attention_mask.device, dtype=torch.bool).unsqueeze(0)
            ...
            if 'PE' in self.ego_status:
                ...
                if input_ids is not None:
                    ...
                    insert_input_ids = torch.tensor([self._pos_indicator_token_index, self._image_token_index]*len_past_pos, device=input_ids.device).unsqueeze(0)
                    insert_attn_mask = torch.ones((1, len_past_pos * 2), device=attention_mask.device, dtype=torch.bool)
        ...
        if self.with_lm_head:
            ...
            for i, input_ids in enumerate(input_ids): # batch level , is 1
                ...
                if self.io_3d_pos:
                    ...
                    pos_indicator_mask = (output_ids[0] == self._pos_indicator_token_index)
                    ...
                    output_ids[0, pos_embedding_index] = self._pos_embedding_token_index
