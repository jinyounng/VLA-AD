# Qwen2.5-VL-3B-Instruct (SpaceDrive: download HF ckpt to ./ckpts, run tools/token_additor.py, set constants.py)
# Same as spacedrive_qwen.py but llm / tokenizer paths and work_dir for 3B.
_base_ = [
...
batch_size = 4
num_epochs = 6 
num_train_samples = 23930  # train samples with full 6-point planning GT
num_iters_per_epoch = num_train_samples // (num_gpus * batch_size)
# Effective batch = num_gpus * batch_size * cumulative_iters (1*4*2 = 8).
...
# llm config
llm_lora_rank=16 
llm_path = 'ckpts/Qwen2.5-VL-3B-Instruct-with-new-special-tokens/'
tokenizer_path = 'ckpts/Qwen2.5-VL-3B-Instruct-with-new-special-tokens/'
...
# save paths
base_path = 'workspace/spacedrive_qwen2.5-3b/'
work_dir = 'workspace/spacedrive_qwen2.5-3b-batch4'
...
############### data ###############
dataset_type = 'CustomNuScenesPlanningDataset'
...
# override default_runtime log interval (50)
log_config = dict(
    interval=20,
    hooks=[
