...
class CustomQwen3VLForConditionalGeneration(Qwen3VLForConditionalGeneration):
    ...
    def forward(
    ...
    ) -> Union[Tuple, CustomQwen3VLCausalLMOutputWithPast]:
        r"""
        ...
        if hasattr(self, 'new_lm_head'):
            ...
            vs = self.config.text_config.vocab_size
            logits = logits.index_copy(
                2,
                torch.arange(vs - 2, vs, device=logits.device),
                new_logits,
            )
        ...
        if labels is not None:
            ...
            loss = self.loss_function(
                ...
                weight=weighted_mask,
                num_items_in_batch=torch.tensor(B),
            )
        ...
        return CustomQwen3VLCausalLMOutputWithPast(  # NOTE: This is used for test Inference
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
...
class CustomQwen3VLModel(Qwen3VLModel):
    """Qwen3-VL backbone with SpaceDrive visual / coord hooks (mirrors CustomQwen2_5_VLModel)."""
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if hasattr(self.language_model, "new_embed_tokens"):
                is_new_token = input_ids >= (vs - 2)
                with torch.no_grad():
                    original_embeds = self.get_input_embeddings()(input_ids)
                new_token_relative_ids = input_ids[is_new_token] - (vs - 2)
