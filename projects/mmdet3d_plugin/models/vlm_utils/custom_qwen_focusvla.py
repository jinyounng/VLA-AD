...
from .focused_visual_context import FocusedVisualContext
...
class CustomQwen2_5_VLCausalLMOutputWithPast(Qwen2_5_VLCausalLMOutputWithPast):
    ...
    focus_vla_stats: Optional[dict] = None


@dataclass
class FocusVLAQwen2_5_VLModelOutputWithPast(Qwen2_5_VLModelOutputWithPast):
    visual_tokens: Optional[torch.FloatTensor] = None


class CustomQwen2_5_VLFocusVLAForConditionalGeneration(Qwen2_5_VLForConditionalGeneration):
    def __init__(self, config, ):
        super().__init__(config)
        self.model = CustomQwen2_5_VLFocusVLAModel(config)
        ...
        self.focus_vla_enabled = False
        self.focus_vla_top_k = 64
        self.focused_visual_context = FocusedVisualContext(self.llm_hidden_dim)

    def configure_focus_vla(
        self,
        enabled: bool = False,
        top_k: int = 64,
        query_mode: str = "mean",
        ...
        self.focus_vla_enabled = enabled
        self.focus_vla_top_k = top_k
        self.focused_visual_context = FocusedVisualContext(
            self.llm_hidden_dim,
    ...
    def _select_focus_visual_tokens(self, visual_tokens, has_gt_planning, num_valid):
        if visual_tokens is None:
            raise RuntimeError("FocusVLA is enabled, but Qwen did not return visual tokens.")
        if visual_tokens.dim() != 3:
            raise RuntimeError(f"FocusVLA expected visual_tokens [B, N_vis, D], got {visual_tokens.shape}")

        batch_size = visual_tokens.shape[0]
        if num_valid == batch_size:
            return visual_tokens

        valid_mask = has_gt_planning.reshape(-1).to(device=visual_tokens.device, dtype=torch.bool)
        if valid_mask.numel() != batch_size:
            raise RuntimeError(
                "FocusVLA cannot align visual tokens with output_pos: "
                f"has_gt_planning has {valid_mask.numel()} entries, visual batch is {batch_size}."
            )
        if int(valid_mask.sum().item()) != num_valid:
            raise RuntimeError(
                "FocusVLA valid-sample mismatch: "
                f"has_gt_planning sum {int(valid_mask.sum().item())}, output_pos batch {num_valid}."
            )
        return visual_tokens[valid_mask]
    ...
    def forward(
            ...
            if not has_gt_planning.any():
                ...
                if planning_only:
                    ...
                    if io_coords_pos is not None:
                        ...
                        if self.focus_vla_enabled:
                            focus_visual_tokens = self._select_focus_visual_tokens(
                                outputs.visual_tokens,
                                has_gt_planning,
                                int(num_valid.item() if hasattr(num_valid, "item") else num_valid),
                            )
                            output_pos = self.focused_visual_context(
                                output_pos,
                                focus_visual_tokens,
                                top_k=self.focus_vla_top_k,
                            )
                            focus_vla_stats = self.focused_visual_context.last_stats
        ...
        if labels is not None and io_coords_pos_mask is not None:
            return CustomQwen2_5_VLCausalLMOutputWithPast(
                ...
                labels = labels,
                focus_vla_stats=focus_vla_stats
            )
...
class CustomQwen2_5_VLFocusVLAModel(Qwen2_5_VLModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        if pixel_values is not None:
            ...
            visual_tokens = image_embeds.reshape(B, -1, image_embeds.shape[-1])
        ...
        output = FocusVLAQwen2_5_VLModelOutputWithPast(
